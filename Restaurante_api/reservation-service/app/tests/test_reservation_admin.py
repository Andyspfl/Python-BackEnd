...
def test_create_reservation(test_client, admin_auth_header):
    create_data = {
        "user_id": 1,
        "restaurant_id": 1,
    ...
    with patch('requests.get') as mock_get:
        ...
        response = test_client.post('/api/reservations', json=create_data, headers=admin_auth_header)
...
def test_get_reservation(test_client, admin_auth_header):
    reservation_id = 1  # Suponemos que la reserva con ID 1 existe

    # Realizamos la petición para obtener la reserva
    response = test_client.get(f'/api/reservations/{reservation_id}', headers=admin_auth_header)
...
def test_update_reservation(test_client, admin_auth_header):
    reservation_id = 1  # Suponemos que la reserva con ID 1 existe
    ...
    with patch('requests.get') as mock_get:
        ...
        response = test_client.put(f'/api/reservations/{reservation_id}', json=update_data, headers=admin_auth_header)
...
def test_delete_reservation(test_client, admin_auth_header):
    reservation_id = 1  # Suponemos que la reserva con ID 1 existe

    # Realizamos la petición para eliminar la reserva
    response = test_client.delete(f'/api/reservations/{reservation_id}', headers=admin_auth_header)
